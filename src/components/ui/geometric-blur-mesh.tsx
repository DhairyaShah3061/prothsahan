...
export interface GeometricBlurMeshProps extends React.HTMLAttributes<HTMLDivElement> {
  autoCycleMs?: number;
}

export default function GeometricBlurMesh({ className, autoCycleMs = 3000, ...props }: GeometricBlurMeshProps) {
  ...
  return (
    <div ref={containerRef} className={cn('relative w-full h-full bg-black overflow-hidden', className)} {...props}>
      <canvas ref={canvasRef} className="w-full h-full" />
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 pointer-events-none text-center">
        <div className="text-white/20 text-xs font-light tracking-widest">{shapes[currentShape].toUpperCase()}</div>
        <div className="text-white/10 text-[10px] font-light mt-1 tracking-wide">SHAPES CHANGE EVERY 3 SECONDS</div>
      </div>
    </div>
  );
}
